...
class Fret_ruler(inkex.Effect):
	...
	def __init__(self):
		...
		self.arg_parser.add_argument("--active_tab", default='ruler', help="Active tab.")
...
###
	def effect(self):
		...
		if self.options.method == 'scala':
			scala_filename = os.path.normpath(os.path.join("scales/", self.options.scala_filename)) #"scales/"+self.options.scala_filename
			# inkex.utils.debug(scala_filename)
			if scala_filename[-4:] != ".scl":
				scala_filename += ".scl"
		# Create group center of view
		translate = 'translate(%s,%s)' % (self.svg.namedview.center[0]-self.options.length*self.convFactor/2, self.svg.namedview.center[1])
		grp_attribs = {inkex.addNS('label','inkscape'):'Fret Ruler', 'transform':translate}
		grp = etree.SubElement(self.svg.get_current_layer(), 'g', grp_attribs)
		page = self.options.active_tab
